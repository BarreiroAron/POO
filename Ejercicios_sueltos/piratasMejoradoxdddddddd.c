...
void creacion_Matriz(int fila, int columna, int l, char *a[][100], int *pf, int *pc, int *tf, int *tc) {
...
void jugar(int fila, int columna, char a[]][], int *pf, int *pc, int tf, int pf) {
...
void imprimirLaMatriz(char a[][], int fila, int columna) {
                /*
...
int main() {
  //Creo una variable entera para las filas de la matriz
    ...
    int intentos = 0;
    char a[][];
    creacion_Matriz(a, filas, columna, &pf, &pc, &tf, &tc);
    imprimirLaMatriz(a, filas, columnas);
    jugar(a, filas, columna, &pf, &pc, tf, tc);
